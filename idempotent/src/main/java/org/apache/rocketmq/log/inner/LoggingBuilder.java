package org.apache.rocketmq.log.inner;
...
public class LoggingBuilder {
    ...
    public static class AsyncAppender extends Appender implements Appender.AppenderPipeline {
        ...
        public AsyncAppender() {
            this.dispatcher = new Thread(new Dispatcher(this, this.buffer, this.discardMap, this.appenderPipeline));
        ...
        private class Dispatcher implements Runnable {
            ...
            public Dispatcher(AsyncAppender parent, List<LoggingEvent> buffer, Map<String, DiscardSummary> discardMap, AppenderPipelineImpl appenderPipeline) {
                this.parent = parent;
                this.buffer = buffer;
                    ...
                    while(isActive) {
                        ...
                        synchronized(this.buffer) {
                            ...
                            if (bufferSize > 0) {
                                ...
                                index = bufferSize;
                                Collection<DiscardSummary> values = this.discardMap.values();

                                DiscardSummary value;
                                for(Iterator var7 = values.iterator(); var7.hasNext(); events[index++] = value.createEvent()) {
                                    value = (DiscardSummary)var7.next();
                                }
